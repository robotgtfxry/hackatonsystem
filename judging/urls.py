...
urlpatterns = [
    # Stary system oceniania (kryteria, Score)
    ...
    # Członkowie jury — zarządzanie (admin)
    ...
    # JEDEN link dla jury — bez logowania
    path('vote/', views.jury_vote_panel, name='jury_vote_panel'),
    path('api/check-session/', views.jury_check_session_api, name='jury_check_session_api'),

    # Skaner QR (admin) — skanuje z ekranu jurora + paruje
    path('scanner/', views.jury_scanner, name='jury_scanner'),
    path('pair/', views.jury_pair_session, name='jury_pair_session'),
    path('unpair/<int:pk>/', views.jury_unpair_session, name='jury_unpair_session'),
    path('clear-sessions/', views.jury_clear_sessions, name='jury_clear_sessions'),

    # Prezentacja — admin kontroluje projekty
